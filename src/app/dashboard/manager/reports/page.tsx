            ...
            <div className="flex flex-wrap items-end gap-md">
              ...
              <div className="pt-2 sm:pt-0">
                <Button 
                  ...
                  className="h-10 gap-2 font-medium text-xs uppercase tracking-widest hover:bg-muted/30 shadow-sm transition-all active:scale-95 text-body"
                >
                  ...
                  >
                        <TableCell className="standard-table-cell">
                          <Link href={`/dashboard/manager/reports/${report.id}`} className="flex flex-col gap-1">
                            <span className="font-normal text-sm text-heading">{report.templateTitle}</span>
                            <span className="text-[10px] text-muted-text font-normal tracking-widest uppercase opacity-60">ID: {report.id.slice(-8)}</span>
                          </Link>
                        </TableCell>
                        <TableCell className="standard-table-cell">
                          <Link href={`/dashboard/manager/reports/${report.id}`} className="block">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-sm font-normal text-body">{report.locationName}</span>
                            </div>
                          </Link>
                        </TableCell>
                        <TableCell className="standard-table-cell">
                          <Link href={`/dashboard/manager/reports/${report.id}`} className="flex flex-col gap-1">
                            <span className="text-sm font-normal text-heading">{report.completedAt ? format(report.completedAt.toDate(), 'MMM d, yyyy') : 'N/A'}</span>
                            <span className="text-[10px] font-normal text-muted-text uppercase tracking-widest opacity-60">Completed</span>
                          </Link>
                        </TableCell>
                        <TableCell className="standard-table-cell text-center">
                          <Link href={`/dashboard/manager/reports/${report.id}`} className="flex justify-center">
                            <Badge className={cn(
                              "px-3 py-1 font-medium text-xs tracking-widest uppercase",
                        ...
                        <TableCell className="standard-table-cell text-right">
                          <Link href={`/dashboard/manager/reports/${report.id}`} className="flex items-center justify-end gap-2 text-muted-text group-hover:text-primary transition-colors">
                              <span className="text-[10px] font-medium uppercase tracking-widest hidden sm:inline">View</span>
